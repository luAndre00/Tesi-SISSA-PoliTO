...
if __name__ == "__main__":
    ...
    path_load = "/scratch/atataran/Tesi-SISSA-PoliTO/Codici/FourierEmbedding/stokes/checkpoint/"
    ...
    parser.add_argument("--mu", help="parameter mu for plotting and for computing the error", type=float)
    ...
    if (args.physic_sampling == 'grid' or args.physic_sampling == 'chebyshev'):
        ...
        opc.discretise_domain(n=int(np.sqrt(1800)), mode=args.physic_sampling, variables=['x', 'y'],
                              locations=['gamma_above', 'gamma_below', 'gamma_right', 'gamma_left'])
    else:
        opc.discretise_domain(n=400, mode=args.physic_sampling, variables=['x', 'y'], locations=['D'])
        opc.discretise_domain(n=1800, mode=args.physic_sampling, variables=['x', 'y'],
                              locations=['gamma_above', 'gamma_below', 'gamma_right', 'gamma_left'])
        # parametric domain
    if (args.parametric_sampling == 'grid' or args.parametric_sampling == 'chebyshev'):
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(10)), mode=args.parametric_sampling, variables=['mu'], locations=['D'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(10)), mode=args.parametric_sampling, variables=['mu'],
                              locations=['gamma_above'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(10)), mode=args.parametric_sampling, variables=['mu'],
                              locations=['gamma_below'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(10)), mode=args.parametric_sampling, variables=['mu'],
                              locations=['gamma_right'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(10)), mode=args.parametric_sampling, variables=['mu'],
                              locations=['gamma_left'])
    else:
        torch.manual_seed(seed);
        opc.discretise_domain(n=10, mode=args.parametric_sampling, variables=['mu'], locations=['D'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=10, mode=args.parametric_sampling, variables=['mu'], locations=['gamma_above'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=10, mode=args.parametric_sampling, variables=['mu'], locations=['gamma_below'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=10, mode=args.parametric_sampling, variables=['mu'], locations=['gamma_right'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=10, mode=args.parametric_sampling, variables=['mu'], locations=['gamma_left'])
    ...
    if args.pipes == 1:
        model = MultiscaleFourierNet_one_pipe(func = torch.nn.Tanh, sigma = args.sigma1, neurons = args.neurons,
                                              ...
                                              input_dimensions = 100, output_dimensions = 100)
    elif args.pipes == 2:
