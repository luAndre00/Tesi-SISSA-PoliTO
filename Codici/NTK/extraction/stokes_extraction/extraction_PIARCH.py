...
if __name__ == "__main__":
    ...
    path_load = "/scratch/atataran/Tesi-SISSA-PoliTO/Codici/NTK/extraction/stokes_extraction/ntk.ckpt"
    ...
    if (args.physic_sampling == 'grid' or args.physic_sampling == 'chebyshev'):
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(400)), mode=args.physic_sampling, variables=['x', 'y'], locations=['state'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(400)), mode=args.physic_sampling, variables=['x', 'y'],
                              locations=['adjoint'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(1800)), mode=args.physic_sampling, variables=['x', 'y'],
                              locations=['gamma_right'])
    else:
        torch.manual_seed(seed);
        opc.discretise_domain(n=400, mode=args.physic_sampling, variables=['x', 'y'], locations=['state'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=400, mode=args.physic_sampling, variables=['x', 'y'], locations=['adjoint'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=1800, mode=args.physic_sampling, variables=['x', 'y'], locations=['gamma_right'])
    # parametric domain
    if (args.parametric_sampling == 'grid' or args.parametric_sampling == 'chebyshev'):
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(10)), mode=args.parametric_sampling, variables=['mu'], locations=['state'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(10)), mode=args.parametric_sampling, variables=['mu'],
                              locations=['adjoint'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=int(np.sqrt(10)), mode=args.parametric_sampling, variables=['mu'],
                              locations=['gamma_right'])
        ...
        torch.manual_seed(seed);
        opc.discretise_domain(n=10, mode=args.parametric_sampling, variables=['mu'], locations=['state'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=10, mode=args.parametric_sampling, variables=['mu'], locations=['adjoint'])
        torch.manual_seed(seed);
        opc.discretise_domain(n=10, mode=args.parametric_sampling, variables=['mu'], locations=['gamma_right'])
    ...
    pinn = PINN.load_from_checkpoint(checkpoint_path=path_load, problem=opc, model=model)
...
#Qui plotto i sample points relativi solamente ai parametri, DA SISTEMARE, USARE LA FUNZIONE DI PINA
