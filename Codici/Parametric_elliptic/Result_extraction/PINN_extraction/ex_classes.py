...
class plot_loss:
    def all_plots(self, val):
        ...
        nomi = ["gamma1","gamma2","gamma3","gamma4","D_loss","mean_loss"]
        for j in range(0,6):
            y = val[:,j]
            plt.loglog(x,y)
            plt.tight_layout()
...
class Scatter_plot:
    def plot_parameter(self, sample_points, str):
        nomi = ['D', 'gamma1', 'gamma2', 'gamma3', 'gamma4']
        for nome in nomi:
            mu1 = sample_points[nome]['mu1']
            mu2 = sample_points[nome]['mu2']
            thickness = 25 * np.ones(mu1.shape) 
            plt.scatter(mu1, mu2, s=thickness, color="r", alpha=1, marker='+')
            plt.xlabel(r'$\mu_1$')
            plt.ylabel(r'$\mu_2$')
            plt.tight_layout()
    ...
    def plot(self, val, x, y, str):
        ...
        plt.imshow(zi, extent=(-1, 1, -1, 1), origin='lower', cmap='viridis', aspect='auto')
        plt.colorbar()
        plt.tight_layout()
    ...
    def plots(self, val, x, y, str, sample_points):
        ...
        if True: #cioè se voglio mettere anche i punti di sample
            for nome in nomi:
                x_sample = sample_points[nome]['x1'].detach().numpy()
                y_sample = sample_points[nome]['x2'].detach().numpy()
                ...
                elif nome == "gamma1" or nome == "gamma2":
