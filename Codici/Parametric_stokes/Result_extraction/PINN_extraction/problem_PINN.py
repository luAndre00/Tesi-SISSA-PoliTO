...
class ParametricStokesOptimalControl(SpatialProblem, ParametricProblem):
    ...
    def momentum_ad_x(input_, output_):
        delta = laplacian(output_, input_, components=['zx'], d=['x', 'y'])
    ...
    def momentum_ad_y(input_, output_):
        delta = laplacian(output_, input_, components=['zy'], d=['x', 'y'])
    ...
    def momentum_y(input_, output_):
        ...
        return -0.1 * delta + grad(output_, input_, components=['p'], d=['y']) + input_.extract( #ATTENZIONE QUESTO SEGNO 
    ...
    def piarch_step(input_, output_):
        return output_.extract(['ux', 'uy'])*0.008 - output_.extract(['zx','zy'])
    ...
    conditions = {
        ...
        'D': Condition(location=CartesianDomain({'x': xrange, 'y': yrange, 'mu': murange}),
                       equation=SystemEquation([momentum_x, momentum_y, continuity, momentum_ad_x, momentum_ad_y, continuity_ad, piarch_step], reduction = "none"))
    }
