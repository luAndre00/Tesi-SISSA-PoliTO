...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", help="extra features", type=int, default=8000)
    ...
    parser.add_argument("--name", help="name for the plots", type=str)
    ...
    plotter = Plotter()
    plotter.plot(solver = pinn, filename = args.name + "_alpha_" + str(alpha.item()) + ".png")
    plotter.plot_loss(trainer = trainer, filename = args.name + '_loss' + str(alpha.item()) + '.png', metrics = ['D_loss', 'gamma1_loss', 'gamma2_loss'], logx = True, logy = True)
