...
class PINN(SolverInterface):
    ...
    def __init__(
        ...
        self.lossVec = []                          #Aggiunto
        self.learning_rates = []                   #Aggiunto
        self.scheduling = scheduling               #Aggiunto
        self.optimizer_kwargs = optimizer_kwargs   #Aggiunto
    ...
    def training_step(self, batch, batch_idx):
        ...
        ################################################################################################################################
        # INSERIRE IN QUESTO PUNTO UNA QUALISASI FORMULA DESIDERATA DI UNO SCHEDULER

        #Estrai il learning rate
        #self.optimizers[0].param_groups[0]['lr'] = self.optimizers[0].param_groups[0]['lr'] * (0.1 ** (len(self.lossVec)) // 30)
        #base_lr/(1+factor*epoche), base_lr 0.01 factor 0.001 -> formula consigliata da Strazzullo
        if self.scheduling == 1:
            self.optimizers[0].param_groups[0]['lr'] = self.optimizer_kwargs['lr'] / (1 + 0.001*len(self.lossVec))
        
        self.lossVec.append(total_loss.item())
        self.learning_rates.append(self.optimizers[0].param_groups[0]['lr'])
        ################################################################################################################################
